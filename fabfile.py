# -*- coding: utf-8 -*-

from fabric.api import *
from fabric.contrib.files import exists, append, upload_template

from fab_settings import *

env.ami = 'ami-82fa58eb'
env.directory = '/home/%s/projects/kopilka' % SSH_USER
env.manage_dir = env.directory + '/src'
env.deploy_user = env.user = SSH_USER
env.activate = 'source %s/ENV/bin/activate' % env.directory
env.www_ssh_key = 'ssh-dss  dsa-key-20080514'

if not env.hosts:
    env.hosts = ['ec2-107-21-102-210.compute-1.amazonaws.com']


def virtualenv(command):
    with cd(env.directory):
        run(env.activate + ' && ' + command)


def init():
    env.deploy_user = env.user = 'ubuntu'

    sudo('apt-get update')
    sudo('apt-get install -y mc lighttpd mysql-client git-core python-setuptools python-dev runit rrdtool memcached libjpeg62-dev')
    sudo('apt-get build-dep -y python-mysqldb')

    if not exists('/home/%s' % SSH_USER):
        sudo('yes | adduser --disabled-password %s' % SSH_USER)
        sudo('mkdir /home/%s/.ssh' % SSH_USER)
        sudo('echo "%s" >> /home/%s/.ssh/authorized_keys' % (env.www_ssh_key, SSH_USER))

    append('/etc/sudoers', '%s  ALL=(ALL) NOPASSWD:/usr/bin/sv' % SSH_USER, use_sudo=True)

    if not exists('/var/log/projects/kopilka'):
        sudo('mkdir -p /var/log/projects/kopilka')
        sudo('chmod 777 /var/log/projects/kopilka')

    if not exists('/etc/lighttpd/conf-available/10-modules.conf'):
        put('tools/lighttpd/10-modules.conf', '/etc/lighttpd/conf-available/10-modules.conf', use_sudo=True)
        sudo('ln -s /etc/lighttpd/conf-available/10-modules.conf /etc/lighttpd/conf-enabled/10-modules.conf', shell=False)

    if not exists('/etc/lighttpd/conf-available/90-kopilka.conf'):
        sudo('touch /etc/lighttpd/conf-available/90-kopilka.conf')
        sudo('chown %s /etc/lighttpd/conf-available/90-kopilka.conf' % SSH_USER)
    if not exists('/etc/lighttpd/conf-enabled/90-kopilka.conf'):
        sudo('ln -s /etc/lighttpd/conf-available/90-kopilka.conf /etc/lighttpd/conf-enabled/90-kopilka.conf', shell=False)

    if not exists('/etc/sv/kopilka'):
        sudo('mkdir -p /etc/sv/kopilka/supervise')
        sudo('touch /etc/sv/kopilka/run')
        sudo('chown %s /etc/sv/kopilka/run' % SSH_USER)
        sudo('chmod 755 /etc/sv/kopilka/run')
        sudo('ln -s /etc/sv/kopilka /etc/service/kopilka', shell=False)

    sudo('mkdir -p /home/%s/projects/kopilka' % SSH_USER)
    sudo('chown -R %(user)s:%(user)s /home/%(user)s' % {'user': SSH_USER})


def production():
    upload()
    environment()
    local_settings()
    lighttpd()
    runit()
    dump()
    migrate()
    restart()


def upload():
    local('git archive -o archive.tar.gz HEAD')
    put('archive.tar.gz', env.directory + '/archive.tar.gz')
    with cd(env.directory):
        run('tar -zxf archive.tar.gz')
        run('rm archive.tar.gz')
    local('del archive.tar.gz')


def environment():
    with cd(env.directory):
        with settings(warn_only=True):
            run('python virtualenv.py ENV')
        virtualenv('pip install -r requirements.txt')


def local_settings():
    with cd(env.manage_dir):
        upload_template(
            'src/local_settings.py.sample',
            'local_settings.py',
            globals(),
            backup=False
        )


def lighttpd():
    run('cp %(directory)s/tools/lighttpd/90-kopilka.conf /etc/lighttpd/conf-available/90-kopilka.conf' % env, shell=False)
#    sudo('/etc/init.d/lighttpd reload', shell=False)


def runit():
    run('cp %(directory)s/tools/runit/run /etc/sv/kopilka/run' % env, shell=False)


def dump():
    with cd(env.directory):
        TMP_FILE = run("date +/tmp/kopilka_backup_%Y%m%d_%H%M.sql.gz")
        run("mysqldump -u %(DATABASE_USER)s -p%(DATABASE_PASSWORD)s -h %(DATABASE_HOST)s %(DATABASE_DB)s | gzip > " % globals() + TMP_FILE)
        run("tools/yandex_narod.sh -l %(DUMP_ACCOUNT_NAME)s@yandex.ru -p %(DUMP_PASSWORD)s " % globals() + TMP_FILE)
        run("rm %s" % TMP_FILE)


def manage_py(command):
    virtualenv('cd %s && python manage.py %s' % (env.manage_dir, command))


def migrate():
    manage_py('syncdb')
    manage_py('migrate')


def restart():
    run('sudo sv restart kopilka')


def local_env():
    with settings(warn_only=True):
        local('c:\\python\\python virtualenv.py ENV --system-site-packages')
    local('ENV\\Scripts\\pip install -r requirements.txt ')


def update_local_db():
    run("mysqldump -u %(DATABASE_USER)s -p%(DATABASE_PASSWORD)s -h %(DATABASE_HOST)s %(DATABASE_DB)s > dump.sql" % globals())
    get("dump.sql", "dump.sql")
    run("rm dump.sql")
    local("mysql -uroot %(DATABASE_DB)s < dump.sql" % globals())
    local("del dump.sql")


def local_migrate():
    local('cd src && ..\\ENV\\Scripts\\python manage.py schemamigration core --auto')
    local('cd src && ..\\ENV\\Scripts\\python manage.py migrate')